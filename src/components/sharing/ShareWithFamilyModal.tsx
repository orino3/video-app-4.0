...
export default function ShareWithFamilyModal({
  ...
  const fetchFamilyMembers = async () => {
    try {
      ...
      // Pre-select primary contacts
      const primaryContacts = (data || [])
        .filter((m) => m.is_primary_contact)
        .map((m) => m.id);
      setSelectedMembers(primaryContacts);
    } catch (error) {
  ...
  const handleShare = async () => {
    ...
    setSharing(true);
    try {
      // Create share records for each selected family member
      const sharePromises = selectedMembers.map((memberId) =>
        supabase.from('family_share_history').insert({
      ...
      const selectedFamilyMembers = familyMembers.filter((m) =>
      ...
      const shareTypeName = {
        ...
        annotation: 'Annotated Clip',
        stats: 'Performance Stats',
      }[shareType];

      const recipients = selectedFamilyMembers
        .map((m) => `${m.full_name} (${m.relationship})`)
        .join(', ');

      alert(
        `${shareTypeName} shared successfully!\n\nShared with: ${recipients}\n\nNote: Email/SMS notifications will be implemented in the next phase.`
      );
