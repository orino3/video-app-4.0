...
export function PlayerManagement({ teamId, teamName, isTeamOwner = false }: PlayerManagementProps) {
  ...
  const fetchPlayerProfiles = async () => {
    try {
      ...
      if (error) throw error;
      console.log('[PlayerManagement] Fetched player profiles:', (data || []).length, 'for team:', teamId);
  ...
  const handleEditSave = async (player: TeamMember) => {
    try {
      ...
      if (editValues.full_name !== player.user.full_name) {
  ...
  const handleSaveProfile = async (profile: PlayerProfile) => {
    try {
      const { error } = await supabase
        ...
        .update({
          full_name: editValues.full_name,
          jersey_number: editValues.jersey_number
            ? parseInt(editValues.jersey_number)
            : null,
        })
        .eq('id', profile.id);
